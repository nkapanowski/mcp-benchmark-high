...
def _success(request_id: str, result: dict, start: int) -> Dict[str, Any]:
    server_duration = _now_ms() - start
    chain_log = result.get("chain_log", [])
    chain_server_ms = sum(s.get("request_duration_ms", 0) for s in chain_log)
    payload: Dict[str, Any] = {
        "request_id":          request_id,
        "status":              "success",
        "result":              result,
        "duration_ms":         server_duration,
        "server_duration_ms":  chain_server_ms if chain_server_ms > 0 else server_duration,
        "ram_rss_mb":          _ram_mb(),
    }
